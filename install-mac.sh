...
set -euo pipefail
IFS=$'\n\t'
# --- ManageEngine Compatibility Block ---
# Get the username of the person currently logged into the GUI
CURRENT_USER=$(scutil <<< "show State:/Users/ConsoleUser" | awk '/Name :/ && ! /loginwindow/ { print $3 }')

# If no user is logged in, exit to avoid installing to the wrong place
if [ -z "$CURRENT_USER" ] || [ "$CURRENT_USER" = "root" ]; then
    echo "Error: No active console user detected. Installation aborted." >&2
    exit 1
fi

# Manually define HOME based on the detected user
